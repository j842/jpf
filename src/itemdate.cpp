...
bool itemdate::set(const boost::gregorian::date &d)
{
    ...
    if (d1 < d0)
        TERMINATE(S() << "Date in the past! Need to update input files: " << d1);

    mDay = _countWeekDays(d0, d1);
    ASSERT(mDay >= 0);
...
// half open interval.
long itemdate::_countWeekDays(boost::gregorian::date d0, boost::gregorian::date d1) const
{
...
boost::gregorian::date itemdate::_day2date(unsigned int day) const
{
    ASSERT(day >= 0);
    boost::gregorian::date d0 = Today();
...
// private.
std::string itemdate::_getstr(const std::locale &fmt) const
{
    if (isForever())
        return "forever";
    ...
    ASSERT(mDay >= 0);
    boost::gregorian::date d0 = _day2date(mDay);
    oss.imbue(fmt);
    oss << d0;
...
std::string itemdate::getStr_nice() const
{
    const std::locale fmt(std::locale::classic(),
...
{
    return getMonthFromStart(_day2date(mDay));
}
...
boost::gregorian::date itemdate::getGregorian() const
{
...
{
    std::ostringstream oss;
    if (((mEnd - mStart)).getAsDurationUInt() > 1)
