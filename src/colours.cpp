...
namespace colours
{
    void turbo_getColour(const float value, float &red, float &green, float &blue)
    {
        ...
        int v = (int)(0.4999 + 255.0 * value);
        if (v > 255)
            v = 255;
        if (v < 0)
            v = 0;
        red = turbo_srgb_floats[v][0];
        green = turbo_srgb_floats[v][1];
        blue = turbo_srgb_floats[v][2];
    }
    ...
    {
        static ColorGradient heatMapGradient;
        return heatMapGradient.getColorAtValue(value, red, green, blue);
    }
