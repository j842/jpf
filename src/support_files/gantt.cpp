...
// created from files in support_files/gantt.
...
/*static*/ bool gantt::output(std::string path)
{
   if (!output_frappe_gantt_css(path)) return false;
   if (!output_frappe_gantt_js(path)) return false;
   if (!output_frappe_gantt_js_map(path)) return false;
   if (!output_frappe_gantt_min_css(path)) return false;
   if (!output_frappe_gantt_min_js(path)) return false;
   if (!output_frappe_gantt_min_js_map(path)) return false;
   if (!output_test_html(path)) return false;
    return true;
}
...
/*static*/ bool gantt::output_frappe_gantt_css(std::string path)
{
    if (path.length()==0) return false;
    if (path[path.length()-1]!='/')
        path.push_back('/');
   std::wofstream os(path+std::string("frappe-gantt.css"));
   if (!os.is_open())
      return false;

   os << 
R"LITERAL(.gantt .grid-background {
  fill: none;
}
.gantt .grid-header {
  fill: #ffffff;
...
/*static*/ bool gantt::output_frappe_gantt_js(std::string path)
{
    if (path.length()==0) return false;
    if (path[path.length()-1]!='/')
        path.push_back('/');
   std::wofstream os(path+std::string("frappe-gantt.js"));
   if (!os.is_open())
      return false;

   os << 
R"LITERAL(var Gantt = (function () {
    'use strict';
...
/*static*/ bool gantt::output_test_html(std::string path)
{
    if (path.length()==0) return false;
    if (path[path.length()-1]!='/')
        path.push_back('/');
   std::wofstream os(path+std::string("test.html"));
   if (!os.is_open())
      return false;
...
.gantt .bar-wrapper {
    ...
    .cc-blue .bar { fill: rgb(43, 48, 122); }
...
<script>
  ...
  },
  {
    ...
    custom_class: 'bar-milestone' // optional
      ...
      id: 'Task 3',
    name: 'Redesign another',
    start: '2017-01-05',
    end: '2017-01-12',
    progress: 0,
    dependencies: 'Task 1, Task 2',
    custom_class: 'bar', // optional
    read_only: true
}
];
var gantt = new Gantt(".gantt", tasks);
var new_height = gantt.$svg.getAttribute('height') - 80;
